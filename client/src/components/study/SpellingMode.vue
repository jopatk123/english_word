      ...
      <div class="spelling-input-area" :style="mode === 'listening' ? 'margin-top: 20px;' : ''">
        ...
        <div v-if="answered" class="spelling-feedback">
          ...
          <div v-else class="spelling-wrong">
            ...
            <div v-if="mode === 'listening'" style="color: #909399; font-size: 14px; margin-top: 4px;">{{ card.word.meaning }}</div>
          </div>
        </div>
      </div>

      <div v-if="!answered" class="spelling-actions">
        <el-button type="primary" @click="$emit('check')" :disabled="!localInput.trim()">确认</el-button>
