...
describe('SessionComplete', () => {
  ...
  const createWrapper = (props = {}) =>
    mount(SessionComplete, {
      props: { sessionStats: defaultStats, hasAgainWords: false, againWordCount: 0, ...props },
      global: {
        stubs: globalStubs,
  ...
  it('hasAgainWords=true 时显示重练按钮', () => {
    const wrapper = createWrapper({ hasAgainWords: true, againWordCount: 3 });
    expect(wrapper.text()).toContain('重练错误单词');
    expect(wrapper.text()).toContain('3');
  });

  it('hasAgainWords=false 时不显示重练按钮', () => {
    const wrapper = createWrapper({ hasAgainWords: false });
    expect(wrapper.text()).not.toContain('重练错误单词');
  });
