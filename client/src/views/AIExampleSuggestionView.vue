        ...
        <div v-if="suggestions.length" class="example-list ai-suggestion-list">
          ...
          >
            ...
            <div class="example-actions">
              ...
              <el-checkbox :model-value="selectedMap[item.sentence]" @change="toggleSelection(item)"
                >选择</el-checkbox
              >
            </div>
          </el-card>
...
<script setup>
  ...
  const fetchBaseData = async () => {
    try {
      const [wordRes, examplesRes] = await Promise.all([getWord(wordId), getExamples({ wordId })]);
  ...
  const generateSuggestions = async () => {
    ...
    // 在生成新建议前，将当前显示的例句添加到拒绝列表
    if (suggestions.value.length) {
      const currentSentences = suggestions.value.map((item) => item.sentence).filter(Boolean);
      rejectedSentences.value = [...new Set([...rejectedSentences.value, ...currentSentences])];
    }
    ...
    try {
      const res = await getAiExampleSuggestions(wordId, settings.value, {
        excludedSentences: rejectedSentences.value,
      });
      ...
      ElMessage.error(
        e?.response?.data?.msg ||
          (e?.code === 'ECONNABORTED' ? 'AI 请求超时，请稍后重试' : '生成例句建议失败')
      );
    } finally {
      loading.value = false;
    }
