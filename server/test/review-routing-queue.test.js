...
describe('GET /review/due', () => {
  ...
  it('待复习单词携带 hasImage 标记，供背单词模式展示记忆图片', async () => {
    await Word.update({ imageExt: 'jpg' }, { where: { id: fixture.wordId } });
    const res = await request(fixture.app).get('/review/due');
    expect(res.status).toBe(200);
    const entry = res.body.data.find((item) => item.wordId === fixture.wordId);
    expect(entry).toBeTruthy();
    expect(entry.word.hasImage).toBe(true);
  });

  it('未上传图片的单词 hasImage 为 false', async () => {
    const res = await request(fixture.app).get('/review/due');
    expect(res.status).toBe(200);
    const entry = res.body.data.find((item) => item.wordId === fixture.wordId);
    expect(entry).toBeTruthy();
    expect(entry.word.hasImage).toBe(false);
  });
