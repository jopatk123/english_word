...
// ── 系统提示共用片段 ───────────────────────────────────────────
const JSON_ONLY_SYSTEM_PROMPT = `你是专业英语学习助手。
...
export const buildRootPrompt = (roots) => {
  const existingRoots = roots.length
    ...
    : '当前还没有任何词根。';
  const existingRootNames = roots.length ? roots.map((r) => r.name).join(', ') : '无';
...
export const buildWordPrompt = (root, words, excludedWords = []) => {
  const existingWords = words.length
    ...
    : '当前该词根下还没有任何单词。';
  const existingWordNames = words.length ? words.map((w) => w.name).join(', ') : '无';

  const extraExcludedWords =
    Array.isArray(excludedWords) && excludedWords.length ? excludedWords.join(', ') : '无';
...
export const buildExamplePrompt = (word, examples, excludedSentences = []) => {
  ...
  const extraExcludedExamples =
    Array.isArray(excludedSentences) && excludedSentences.length
  ...
  return {
    systemPrompt: JSON_ONLY_SYSTEM_PROMPT,
    userPrompt: `为单词【${word.name}】生成1条**全新、不重复**的简短常用英文例句+中文翻译。
...
规则（必须严格遵守）：
1. 句子口语化、简短且句中的单词常用、简单、易学，能体现该单词的日常用法和语境。
2. **绝对不能与以下例句相同或高度相似**：
${existingExamples}
3. **还要避开本次已生成过（但未保存）的例句**：
${extraExcludedExamples}
4. 若确实已无更多可推荐，返回空数组，hasMore: false。
5. **禁止返回 reason 和 remark 字段**。
6. 只返回标准JSON，格式如下：
{
  "message": "简短说明",
...
4. ${
      singleExample
        ? '只生成 1 条日常常用英文例句，句子口语化、简短、自然、易学，且能体现该单词的典型用法。'
        : '根据词性数量生成相应数量的常用英文例句，每条例句口语化、简短且句中的单词常用、简单、易学，能体现不同词性或典型用法。'
    }
5. **绝对不能与以下例句相同或高度相似**：\n${excludedText}
6. 只返回标准JSON，格式如下：
{
