...
- AI 辅助：词根建议、单词建议、例句建议、单词分析、句子分析、模型自动发现、跳过思考模式
...
AI 配置页（`/ai/settings`）支持多厂商切换、模型选择、温度调节与跳过思考开关。配置分为服务端存储与浏览器本地存储两部分：

- **服务端加密存储**：各厂商 API Key 使用 `AI_SETTINGS_SECRET` 加密后保存到数据库，浏览器仅持有掩码摘要，密钥明文不落 localStorage。
- **浏览器本地存储**：厂商选择、Base URL、模型、温度、跳过思考开关、自动拉取的模型列表与手动添加的自定义模型等非敏感偏好，按厂商独立保存于 localStorage。
...
部分模型（DeepSeek-Reasoner/R1/V4 系列（含 V4-Flash/V4-Pro）、Qwen3/QwQ、OpenAI o1/o3/o4/gpt-5、Claude 3.7+/4/4.5、GLM-Z1/4.5/4.6、Kimi-K1/K2、Doubao-Seed 等）会产生隐藏的 reasoning tokens，导致响应慢、token 浪费。开启"跳过思考"开关后，后端会按厂商注入禁用参数：
...
开关按厂商独立保存，仅对识别为思考模型的有效，非思考模型不受影响。前端会以 tag 提示当前模型是否为思考模型，但不阻塞保存与调用。

未开启"跳过思考"时，思考模型的 reasoning tokens 同样计入 `max_tokens` 输出额度，因此后端对思考模型自动放宽输出上限（1800 → 8192），避免推理耗尽额度导致正文为空。若仍出现"AI 输出被截断或仅返回思考过程"的报错，建议开启"跳过思考"。
