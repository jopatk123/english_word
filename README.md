...
1. 选择厂商并填写 Base URL 与 API Key 后，组件挂载、厂商切换、API Key 输入框失焦（500ms 防抖）时会**静默**调用厂商 `/models` 端点拉取可用模型，写入"自动获取的模型"分组。
...
部分模型（DeepSeek-Reasoner/R1/V4-Pro、Qwen3/QwQ、OpenAI o1/o3/o4/gpt-5、Claude 3.7+/4/4.5、GLM-Z1/4.5/4.6、Kimi-K1/K2、Doubao-Seed 等）会产生隐藏的 reasoning tokens，导致响应慢、token 浪费。开启"跳过思考"开关后，后端会按厂商注入禁用参数：
